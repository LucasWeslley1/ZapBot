...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:Lucas‍Weslley 🥶\n' 
            + 'ORG:Dono do ZapBot;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=559188170473:+55 (91) 8817-0473 \n' 
            + 'END:VCARD'
...
async function starts() {
	...
	tiringa.on('qr', () => {
		console.log(color('👆'), color(' Escanear o código acima para iniciar ZapBot!'))
	})

	fs.existsSync('./tiringa.json') && tiringa.loadAuthInfo('./tiringa.json')
	tiringa.on('connecting', () => {
		start('2', 'Conectando ao ZapBot...')
	})
	tiringa.on('open', () => {
		success('2', 'ZapBot conectado!!!')
	})
    ...
    tiringa.on('CB:Blocklist', json => {
      ...
      tiringa.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = [`559188170473@s.whatsapp.net`]
			const ownerNumberB = []
...
//_TIPO DE USUÁRIO
if (groupAdmins) {prema = 'Ademir🤓'}if (isPremium) {prema = 'Premium😎'} if (isOwner) {prema = 'El Lucas🕴🏽'} if (isUser) {prema = 'Registrado 🥳'} else {var prema = 'Membro comum🗿'}
...
if ((budy === 'oi bot') || (budy === 'OI BOT') || (budy === 'Oi Bot')) {
...
tiringa.updatePresence(from, Presence.composing)
reply("você falou em Seqsu?KKKKKKKKKKKKKKKKKKKKKKKKKKKKKK")
}
if (budy === "pnc") {
tiringa.updatePresence(from, Presence.composing)
reply(`vsfd ${pushname}`)
...
if ((budy.includes('Pedro')) || (budy.includes('PEDRO'))) {
reply('Lixo')
}

if (budy.match('tiringnsjxa')) {
...
case 'Lucas':
case 'LUCAS':
if (!isGroup) return reply( 'Me obrigue a fazer figuras L-lucas-chan 😔👉👈')
break

case 'pedro':
case 'Pedro':
case 'PEDRO':
if (!isGroup) return reply( 'Oin pedro, poderia depositar R$3,000 na minha conta?🥺👉👈')
break
...
tiringa.sendMessage('55918870473@s.whatsapp.net', options, text, {quoted: mek})
...
case 'Lucas':
case 'lucas':
await tiringa.sendMessage(from, {displayname: "Lucas🧙‍♂️", vcard: vcard}, MessageType.contact)
break
...
.on('end', function () {
console.log('Finish')
exec(`webpmux -set exif ${addMetadata('Zap-BOT', 'LucasW.')} ${ran} -o ${ran}`, async (error) => {
...
.on('end', function () {
console.log('Finish')
exec(`webpmux -set exif ${addMetadata('Zap-BOT', 'Lucas')} ${ran} -o ${ran}`, async (error) => {
if (error) return reply(ptbr.stick())
tiringa.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
fs.unlinkSync(media)
fs.unlinkSync(ran)
...
fs.unlinkSync(ranp)
if (err) return reply(ptbr.stick())
exec(`webpmux -set exif ${addMetadata('Zap-BOT', 'Lucas')} ${ranw} -o ${ranw}`, async (error) => {
if (error) return reply(ptbr.stick())
...
mentions('╭╾╼◐⚋ ༒ᴍᴇɴᴄɪᴏɴᴀʀ ᴛᴏᴅᴏs ༒⚋◑╾╼╮\n║➸'+todos+'╰╾╼◐⚋⚋ ༒ LucasWeslley ༒ ⚋⚋◑╾╼╯', members_id, true)
break

//_LIMPA TODOS OS CHATS
case 'clearall':
if (!isOwner) return reply('Só o Lucas pode fazer isso')
...
if (!isNsfw) return reply(ptbr.nsfw())
reply(ptbr.waitsfw())
res = axios.get(`https://cdn.nekos.life/boobs/boobs_284.gif`).then(res => {
imageToBase64(res.data.url)
.then(
(ress) => {
var buf = Buffer.from(ress, 'base64')
